 ...
 * @param {Map<string, Object>} loadedRules The rule definitions which were used.
 * @returns {IterableIterator<Object>} Used deprecated rules.
 */
function *iterateRuleDeprecationWarnings(usedConfigArrays, loadedRules) {
    ...
    function getRule(ruleId) {
        for (const configArray of usedConfigArrays) {
            const rule = configArray.getPluginRule(ruleId);

            if (rule) {
                return rule;
            }
        }
        return loadedRules.get(ruleId) || null;
    }
    ...
    constructor(providedOptions) {
        ...
        if (options.fix && options.fixTypes && options.fixTypes.length > 0) {
            ...
            options.fix = message => {
                ...
                const rule = (
                    (lastConfigArray && lastConfigArray.getPluginRule(message.ruleId)) ||
    ...
    executeOnText(text, filename, warnIgnored) {
        ...
        if (filename && ignoredPaths.contains(resolvedFilename)) {
            ...
            usedDeprecatedRules.push(
                ...iterateRuleDeprecationWarnings([config], linter.getRules())
            );
        }
