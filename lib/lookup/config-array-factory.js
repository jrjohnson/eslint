        ...
        try {
            ...
            // Shareable config ------------------------------------------------
                ...
                const configFilePath = ModuleResolver.resolve(
                    request,
                    importerPath || path.join(this._cwd, ".eslintrc")
                );

                yield* this._loadConfigData(configFilePath, { name });
            }
        } catch (error) {
            error.message += `\nReferenced from: ${importerPath || path.join(this._cwd, ".eslintrc")}`;
            throw error;
        }
    }
    ...
    _loadParser(nameOrPath, importerPath, importerName) {
        ...
        try {
            ...
            if (error && error.code === "MODULE_NOT_FOUND") {
                ...
                error.message = `Failed to load parser ${nameOrPath}: ${error.message} (relative to ${importerPath || path.join(this._cwd, ".eslintrc")})`;
            }

            // If the parser name is "espree", load the espree of ESLint for backward compatibility.
