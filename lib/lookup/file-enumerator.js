/**
 * @fileoverview `FileEnumerator` class provides the functionality that
 * enumerates every file which is matched by given glob patterns and that
 * configuration.
 *
 * `FileEnumerator` class provies two methods:
 *
 * - `FileEnumerator#iterateFiles(patterns)`
 ...
 * - `FileEnumerator#getConfigArrayForFile(filePath)`
 ...
 *
 * 1. Look up target files.
 * 2. Look up configuration files with handling cascading.
 *
 * Then it yields both in parallel.
 *
 * `FileEnumerator` class loads configuration files before iterate target files,
 * so we can use the configuration files to determine target files.
 */
"use strict";
 ...
 * @property {ConfigData|null} cliConfigData The config by CLI options. This is prior to regular config files.
 * @property {ConfigArrayFactory} configArrayFactory The factory for config arrays.
...
function isGlobPattern(pattern) {
    if (path.sep === "\\") {
        pattern = pattern.replace(/\\/gu, "/"); // eslint-disable-line no-param-reassign
    }
 ...
 *
 * Use a pseudo plugin to define rules of `--rulesdir`, so we can validate the
 * rule's options with only information in the config array.
 *
 * @param {string[]} rulePaths The value of `--rulesdir` option.
 * @param {string} cwd The CWD.
 * @returns {ConfigArrayElement} The config array element which has the loaded rules.
 */
function createConfigArrayElementForRulesPaths(rulePaths, cwd) {
    return new ConfigArrayElement(
        {
            plugins: {
                "": new LoadedEntity({
                    definition: {
...
class AllFilesIgnoredError extends Error {
     ...
     */
    constructor(pattern, globDisabled) {
        super(`All files matched by '${pattern}' are ignored${globDisabled ? " (glob was disabled)" : ""}.`);
    ...
    constructor({
        ...
        if (rulePaths && rulePaths.length > 0) {
            baseConfigArray.push(
                createConfigArrayElementForRulesPaths(rulePaths, cwd)
            );
        }
    ...
    *_iterateFilesRecursive(
    ...
    _loadConfigInAncestorsRecursive(filePath) {
        ...
        try {
            ...
            if (configArray && configArray.isRoot()) {
     ...
     * @private
     */
    _finalizeConfigArray(parentConfigArray, directoryPath) {
        const {
            cliConfigData,
            configArrayFactory,
            cwd,
        ...
        let finalConfigArray = finalizeCache.get(parentConfigArray);

        if (!finalConfigArray) {
            finalConfigArray = parentConfigArray;

            // Load `--config`
            if (specificConfigPath) {
                finalConfigArray = configArrayFactory.loadFile(
                    specificConfigPath,
                    { name: "--config", parent: parentConfigArray }
                );
            }
            ...
            if (useEslintrc && finalConfigArray.length === 0) {
                ...
                const personalConfig = configArrayFactory.loadOnDirectory(
                    os.homedir(),
                    { name: ".personal" }
                );

                if (personalConfig) {
                    finalConfigArray = personalConfig;
                }
            }
            ...
            if (cliConfigData) {
                finalConfigArray = configArrayFactory.create(
                    cliConfigData,
                    {
                        filePath: path.join(cwd, ".cli"),
                        parent: finalConfigArray
                    }
                );
            }

            finalizeCache.set(parentConfigArray, finalConfigArray);
        }
