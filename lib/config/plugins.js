...
class Plugins {
     ...
     * @param {string} pluginName The name of the plugin to load.
     * @param {string} [relativeTo=""] The location to search for the plugin from.
     * @returns {void}
     * @throws {Error} If the plugin cannot be loaded.
     */
    load(pluginName, relativeTo = "") {
        const longName = naming.normalizePackageName(pluginName, "eslint-plugin");
        const shortName = naming.getShorthandName(longName, "eslint-plugin");
        let plugin = null;
        ...
        if (!this._plugins[shortName]) {
            try {
                let resolutionPaths = require.paths;

                if (relativeTo !== "") {
                    debug(`Using relative path for loading plugin "${shortName}"`);
                    resolutionPaths = [path.join(relativeTo, "node_modules")];
                }

                const fullPluginPath = require.resolve(longName, { paths: resolutionPaths });
                debug(`Loading plugin "${shortName}" from "${fullPluginPath}`);
                plugin = require(fullPluginPath);
