 ...
 * @param {ConfigArrayElement} element The config array element to validate.
 * @param {function(ruleId:string): Object} ruleMapper A map from strings to loaded rules.
 * @param {function(envId:string): Object} envMapper A map from strings to loaded envs.
 * @returns {void}
 */
function validateConfigArrayElement(element, ruleMapper, envMapper) {
    ...
    validateEnvironment(
        element.env,
        element.name,
        id => envMapper(id) || builtInEnvMap.get(id) || null
    );
    validateRules(
        element.rules,
        element.name,
        id => ruleMapper(id) || builtInRuleMap.get(id) || null
    );
}
